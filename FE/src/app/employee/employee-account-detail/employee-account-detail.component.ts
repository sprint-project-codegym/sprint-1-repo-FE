...
​
  update() {
    if (this.formUpdateEmployee.valid) {
      this.personalInfoService.updateEmployee(this.formUpdateEmployee.value).subscribe(
        data => {
        ...
        error => {
          this.toastr.error("Trùng email hoặc số điện thoại, vui lòng nhập lại!", "Thất bại: ", {
            timeOut: 2500,
            extendedTimeOut: 1500
          });
        }
